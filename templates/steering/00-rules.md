...
8. Agent parity: every agent you use (e.g. Kiro and Claude Code) must load the same context. ccb-parity-check.sh reports PASS/FAIL at each session start; on FAIL, fix wiring before trusting the session. Change one agent's wiring and the other's in the same step.
