...
- **H2 (quality non-inferiority):** Tiered context achieves task-success rate **not worse** than monolithic by a margin delta=2pp on SWE-bench-Verified. (Non-inferiority test.)
...
- **H4 (auto-tiering):** An automatic tier assignment (access-frequency classifier) yields cost/quality within a tolerance epsilon of an oracle/manual tiering. (This is the algorithmic contribution.)
...
| C3 tiered-auto | C2 but tiers assigned by the automatic classifier (Section tier_assign) |
...
- **Sample size:** power analysis for a non-inferiority test at delta=2pp, alpha=0.05, power=0.8 -> target N tasks (compute in analysis.py). Report achieved power.
...
- Non-inferiority: one-sided CI on (C2 - C1) success rate vs -delta.
- Cost: Wilcoxon signed-rank on paired token counts.
- Multiple-comparison correction (Holm) across H1 - H4.
- Report effect sizes + CIs, not just p-values. Pre-register delta, epsilon, alpha before running.
